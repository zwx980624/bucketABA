...
def read_claim2vector_embedding_file_no_vector(path, files):
    ...
    claims_counter = 0
    num_claim = 0
    code_freq_at_first_claim = np.zeros(num_dim + 2) # num_dim是item数，冷启动频率？这个论文里提出的一个贡献点
...
def train(input_variable, target_variable, encoder, decoder, codes_inverse_freq, encoder_optimizer, decoder_optimizer,
    ...
    for ei in range(input_length - 1):
        if ei == 0:
            continue
        encoder_output, encoder_hidden = encoder(input_variable[ei], encoder_hidden) # here encoder
...
def main(argv):
    ...
    files = ['./data/TaFang_history.csv', './data/TaFang_future.csv']
    ...
    data_chunk, input_size, code_freq_at_first_claim = read_claim2vector_embedding_file_no_vector(path, files) # 把样本读到data_chunk
    codes_freq = get_codes_frequency_no_vector(data_chunk[past_chunk], input_size, data_chunk[future_chunk].keys()) # 数一下item在历史样本中的出现频率
    training_key_set, validation_key_set, test_key_set = partition_the_data_validate(data_chunk, list(data_chunk[future_chunk]), next_k_step) # 不能按用户划分训练测试集
